...
class JeuDeLaVie {
    ...
    static void afficherPlateau (int [][] tableau,int generation){ /* Affiche le plateau du jeu, avec "." représentant une cellule morte et "#" une cellule vivante */
	System.out.println(generation);                          /*  et la génération*/
	...
	if(a!=voisin.length-1){
	    nbvoisinvivant=voisinVivantOuMort(voisin,a+1,b);                           /* Verifie si la cellule à droite est vivante ou pas */
	}
	if (a!=0){
       	    nbvoisinvivant=nbvoisinvivant+ voisinVivantOuMort(voisin,a-1,b);            /* Verifie si la cellule à gauche est vivante ou pas */
	}
	...
	if (a!=0){
	    if (b!=voisin[0].length-1){
		nbvoisinvivant=nbvoisinvivant+ voisinVivantOuMort(voisin,a-1,b+1);          /* Verifie si la cellule en bas à gauche est vivante ou pas */
	    }
	    if (b!=0){
		nbvoisinvivant=nbvoisinvivant+ voisinVivantOuMort(voisin,a-1,b-1);          /* Verifie si la cellule en haut à gauche est vivante ou pas */
	    }
	}
    ...
    static int mortOuVivant (int nombrevoisinvivant,int[][] tab2, int c, int d){               /*Si une cellule vivante et est entourée de 2 ou 3 cellules vivante alors elle  */
	if (tab2[c][d]==1){                                                         /* reste en vie, sinon elle meurt.  */
	    if (nombrevoisinvivant!=2 ||  nombrevoisinvivant!=3){                   /* Si une cellule est morte et est entourée de exactement 3 cellules vivante  */
	...
	int génération, longueur, largeur, nbinitialisation, colonne, ligne,nbvoisinenvie,pause;
	int [][] plateau;
	génération=0;
	...
	afficherPlateau(plateau,génération);
	génération=1;
	System.out.println("");
	...
	afficherPlateau(plateau,génération);
	while(génération>0){           /*Programme qui ne s'arrêtera pas*/
